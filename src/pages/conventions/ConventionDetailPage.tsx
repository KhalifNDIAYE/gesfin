...
export default function ConventionDetailPage() {
  ...
  if (isLoading) {
    return (
      <AppLayout title="Détail Convention" subtitle="Chargement...">
  ...
  if (!convention) {
    return (
      <AppLayout title="Détail Convention" subtitle="Non trouvée">
        <div className="p-6">Convention non trouvée</div>
      </AppLayout>
    );
  }
  ...
  // Export data
  const replenishmentsExportData = replenishments?.map(r => ({
    code: r.code,
    requestDate: r.request_date,
    amount: r.amount,
    receivedDate: r.received_date,
    status: statusLabels[r.status]?.label || r.status,
  })) || [];

  const paymentsExportData = directPayments?.map(p => ({
    code: p.code,
  ...
  const reportsExportData = financialReports?.map(r => ({
    code: r.code,
    type: reportTypeLabels[r.report_type],
    periodStart: r.period_start,
    periodEnd: r.period_end,
    totalExpenses: r.total_expenses,
    replenishmentRequested: r.replenishment_requested,
    status: statusLabels[r.status]?.label || r.status,
  })) || [];

  return (
    <AppLayout title="Détail Convention" subtitle={convention.name}>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                <div className="flex gap-2">
                  <PermissionGate module="conventions" permission="export">
                    <TableExportButtons
                      data={replenishmentsExportData}
                      columns={[
                        { key: "code", label: "Code" },
                        { key: "requestDate", label: "Date demande", format: (v) => format(new Date(v), "dd/MM/yyyy", { locale: fr }) },
                        { key: "amount", label: "Montant", format: (v) => formatAmount(v, convention.currency?.symbol) },
                        { key: "receivedDate", label: "Date réception", format: (v) => v ? format(new Date(v), "dd/MM/yyyy", { locale: fr }) : "-" },
                        { key: "status", label: "Statut" },
                      ] as ExportColumn[]}
                      filename={`CONVENTION-${convention.code}-REAPPRO-${today}`}
                      title={`Réapprovisionnements - ${convention.code}`}
                      subtitle={`${replenishments?.length || 0} demandes`}
                    />
                  </PermissionGate>
                  <PermissionGate module="conventions" permission="create">
                  ...
                  <PermissionGate module="conventions" permission="export">
                    <TableExportButtons
                      ...
                      columns={[
                        ...
                        { key: "amount", label: "Montant", format: (v) => formatAmount(v, convention.currency?.symbol) },
                        { key: "requestDate", label: "Date demande", format: (v) => format(new Date(v), "dd/MM/yyyy", { locale: fr }) },
                        { key: "paymentDate", label: "Date paiement", format: (v) => v ? format(new Date(v), "dd/MM/yyyy", { locale: fr }) : "-" },
                        { key: "status", label: "Statut" },
                      ] as ExportColumn[]}
                      filename={`CONVENTION-${convention.code}-PAIEMENTS-${today}`}
                      title={`Paiements directs - ${convention.code}`}
                      subtitle={`${directPayments?.length || 0} paiements`}
                    />
                  </PermissionGate>
                  <PermissionGate module="conventions" permission="create">
                  ...
                  <PermissionGate module="conventions" permission="export">
                    <TableExportButtons
                      ...
                      columns={[
                        { key: "code", label: "Code" },
                        { key: "type", label: "Type" },
                        { key: "periodStart", label: "Début", format: (v) => format(new Date(v), "dd/MM/yyyy", { locale: fr }) },
                        { key: "periodEnd", label: "Fin", format: (v) => format(new Date(v), "dd/MM/yyyy", { locale: fr }) },
                        { key: "totalExpenses", label: "Total dépenses", format: (v) => formatAmount(v, convention.currency?.symbol) },
                        { key: "replenishmentRequested", label: "Réappro. demandé", format: (v) => formatAmount(v, convention.currency?.symbol) },
                        { key: "status", label: "Statut" },
                      ] as ExportColumn[]}
                      filename={`CONVENTION-${convention.code}-RAPPORTS-${today}`}
                      title={`Rapports financiers - ${convention.code}`}
                      subtitle={`${financialReports?.length || 0} rapports`}
                    />
                  </PermissionGate>
                  <PermissionGate module="conventions" permission="create">
