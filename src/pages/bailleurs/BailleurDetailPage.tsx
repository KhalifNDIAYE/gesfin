...
export default function BailleurDetailPage() {
  ...
  if (isLoading) {
    return (
      <AppLayout title="Détail Bailleur" subtitle="Chargement...">
  ...
  if (!bailleur) {
    return (
      <AppLayout title="Détail Bailleur" subtitle="Non trouvé">
        <div className="p-6">Bailleur non trouvé</div>
      </AppLayout>
    );
  }
  ...
  const conventionsExportData = conventions?.map(c => ({
    code: c.code,
    name: c.name,
    totalAmount: c.total_amount,
    disbursedAmount: c.disbursed_amount,
    rate: c.total_amount ? ((c.disbursed_amount / c.total_amount) * 100).toFixed(0) + "%" : "0%",
    closingDate: c.closing_date,
    status: statusLabels[c.status]?.label || c.status,
    currencySymbol: c.currency?.symbol || "FCFA",
  })) || [];

  return (
    <AppLayout title="Détail Bailleur" subtitle={bailleur.name}>
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <PermissionGate module="bailleurs" permission="export">
              <TableExportButtons
                data={conventionsExportData}
                columns={[
                  { key: "code", label: "Code" },
                  { key: "name", label: "Nom" },
                  { key: "totalAmount", label: "Montant", format: (v) => formatAmount(v) + " FCFA" },
                  { key: "disbursedAmount", label: "Décaissé", format: (v) => formatAmount(v) + " FCFA" },
                  { key: "rate", label: "Taux" },
                  { key: "closingDate", label: "Clôture", format: (v) => v ? format(new Date(v), "dd/MM/yyyy", { locale: fr }) : "-" },
                  { key: "status", label: "Statut" },
                ] as ExportColumn[]}
                filename={`BAILLEUR-${bailleur.code}-CONVENTIONS-${today}`}
                title={`Conventions du bailleur ${bailleur.name}`}
                subtitle={`${conventions?.length || 0} conventions`}
              />
            </PermissionGate>
          </CardHeader>
          <CardContent>
